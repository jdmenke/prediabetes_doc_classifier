...
for sn in sections_name:
        if sn not in 'wp_':
                col_name = f"{sn}extracted_sents"
                header.append(col_name)
header.extend(['whole_paper'])
...
def processpaper(input, base_name):
        ...
        abstract_data = [0] * (len(name_modifiers) * len(entities))
        intro_data = [0] * (len(name_modifiers) * len(entities))
        methods_data = [0] * (len(name_modifiers) * len(entities))
        results_data = [0] * (len(name_modifiers) * len(entities))
        discussion_data = [0] * (len(name_modifiers) * len(entities))
        conclusion_data = [0] * (len(name_modifiers) * len(entities))
        table_data = [0] * (len(name_modifiers) * len(entities))
        with open(input) as fp:                
                contents = fp.read()
                paper_dict = json.loads(contents)
                table_text = paper_dict['all_tables']
                table_data, table2extract = search4patterns(str(table_text))
        ...
        for section in normalized_dict.keys():
                ...
                if section in ['abstract', 'introduction', 'methods', 'results', 'discussion', 'conclusions']:
                        ...
                        whole_paper_text += section_content
                        if section in 'abstract':
                                abstract_data, text2extract = search4patterns(section_content)
                                a_extracted_sents += text2extract
                        elif section in 'introduction':
                                intro_data, text2extract = search4patterns(section_content)
                                i_extracted_sents += text2extract
                        elif section in 'methods':
                                methods_data, text2extract = search4patterns(section_content)
                                m_extracted_sents += text2extract
                        elif section in 'results':
                                results_data, text2extract = search4patterns(section_content)
                                r_extracted_sents += text2extract
                        elif section in 'discussion':
                                discussion_data, text2extract = search4patterns(section_content)
                                d_extracted_sents += text2extract
                        elif section in 'conclusion':
                                conclusion_data, text2extract = search4patterns(section_content)
                                c_extracted_sents += text2extract
                else:
                        pass
        
        whole_paper.extend(abstract_data)
        whole_paper.extend(intro_data)
        whole_paper.extend(methods_data) 
        whole_paper.extend(results_data) 
        whole_paper.extend(discussion_data)
        whole_paper.extend(conclusion_data)
        whole_paper.extend(table_data)

        ai_data = np.add(abstract_data, intro_data)
        mr_data = np.add(methods_data, results_data)
        dc_data = np.add(discussion_data, conclusion_data)
        dct_data = np.add(dc_data, table_data)
        aimr_data = np.add(ai_data, mr_data)
        wp_data = np.add(dct_data, aimr_data)
        whole_paper.extend(wp_data)

        whole_paper.append(a_extracted_sents)
        whole_paper.append(i_extracted_sents)
        whole_paper.append(m_extracted_sents)
        whole_paper.append(r_extracted_sents)
        whole_paper.append(d_extracted_sents)
        whole_paper.append(c_extracted_sents)
        whole_paper.append(table2extract)
        whole_paper.append(whole_paper_text)
...
def search4patterns(section_text=str):
        ...
        for sentence in doc:
                if sentence not in section_cache:
                        ...
                        if len(np.flatnonzero(np.array(any_positive))) > 0:
                                text2extract += sentence
        ...
        return results_list, text2extract
...
def process_directory(input_directory, output_directory):
        ...
        regex_analysis = os.path.join(output_directory, 'training_testing_data_sectional.csv')
